 ...
 */
/**
 *  queryrelativelist.js
 *  功能    查询-授权人列表视图
 *  作者    xxx
 *  日期    2018-09-xx
 *  当前路由 /queryrelativelist
 *  前一路由 /uanindex
 *  后一路由 /queryrelativedetail
 *  输出变量 window.app.queryRelativeListV
 * 
 * 
 */
;(function(factory) {
...
\
    ...
    var queryRelativeListDomString = kendo.template(queryRelativeListDomTemplate)({});
  
    // queryRelativeList视图view modal
    var queryRelativeListVM = kendo.observable({
      // request
      appId: '',
      userId: '',
      // respond
      ...
      show: function () {
        ...
        .done(function(res) {
          if(res.statusCode != '200') { 
            ...
            // 跳转路由至 /uanindex
            app.redirectTo('/uanindex');
            return;
          }
          // 受助人列表返回成功，将返回的数据写入
          var listVM = app.queryRelativeListV.model,
                data = res.data,
                i;
    ...
    // queryRelativeList视图view，从字符串变量queryRelativeListDomString中加载DOM结构，绑定ViewModel
    var queryRelativeListV = new kendo.View(queryRelativeListDomString, {
      model: queryRelativeListVM, 
      init: queryRelativeListVM.init.bind(queryRelativeListVM),
      show: queryRelativeListVM.show.bind(queryRelativeListVM),
      hide: queryRelativeListVM.hide.bind(queryRelativeListVM),
      prevPage: queryRelativeListVM.prevPage.bind(queryRelativeListVM),
      queryRelativeAsset: queryRelativeListVM.queryRelativeAsset.bind(queryRelativeListVM),
    });
  
    // queryRelativeList视图路由，渲染指定view
    router.route("/queryrelativelist", function () {
      layout.showIn(layoutSelector,queryRelativeListV);
    });
  
      // 输出至全局变量app
      window.app = $.extend(true, app, {
        'queryRelativeListV': queryRelativeListV
      });
