...
public abstract class AbstractInputHeader extends AbstractPojo {
	...
	private String uniqueId; // 统一流水号(请求终端设备产生，全流程唯一号),要求实现 (不应删除此字段)
	private String preUniqueId;
	private String appId; // 请求方标识,推荐实现 (如不需要可删除此字段)
	private String userId; // 用户标识码,可选择性实现 (如不需要可删除此字段)
	private String trToken; // 交易请求口令,可选择性实现 (如不需要可删除此字段)
	private String shardingDataSource; // 碎片化的数据访问规则，用于分库分表等场景
